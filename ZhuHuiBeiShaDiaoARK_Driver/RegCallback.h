...
PNOTIFY_INFO CountCmpCallbackAfterXP(ULONG64* pPspLINotifyRoutine)
{
	ULONG				sum = 0;
	ULONG64				dwNotifyItemAddr;	
	PLIST_ENTRY			notifyList;
	PCM_NOTIFY_ENTRY	notify;
	PNOTIFY_INFO pNotifyInfo = NULL;
	SYSTEM_MODULE Sysmodule = { 0 };

	pNotifyInfo = ExAllocatePool(NonPagedPool, sizeof(NOTIFY_INFO) * 100);
	if (pNotifyInfo == NULL)
		return NULL;
	...
	{		
		...
		if (MmIsAddressValid(notify))
		{
			if (MmIsAddressValid((PVOID)(notify->Function)) && notify->Function > 0x8000000000000000)
			{
				...
				pNotifyInfo[sum].CallbacksAddr = notify->Function;
				pNotifyInfo[sum].CallbackType = 3; // cmp
				pNotifyInfo[sum].Cookie = notify->Cookie.QuadPart;
				memset(&Sysmodule, 0, sizeof(SYSTEM_MODULE));
				if (NT_SUCCESS(getSystemImageInfoByAddress(pNotifyInfo[sum].CallbacksAddr, &Sysmodule)) &&
					strlen(Sysmodule.ImageName) < MAX_PATH)
				{
					RtlCopyMemory(pNotifyInfo[sum].ImgPath, Sysmodule.ImageName, MAX_PATH);
				}
...
PNOTIFY_INFO EnumCmCallback64()
{
	PNOTIFY_INFO pNotify = NULL;
	//test to add my reg callback
	//CmRegisterCallback(MyRegistryCallback, NULL, &Cookie);
	//DbgPrint("[MY FUNCTION]: %p\n",(PVOID)MyRegistryCallback);
	//DbgPrint("[MY COOKIE]: %p\n",(PVOID)Cookie.QuadPart);
	//get CmCallbackListHead address
	CmCallbackListHead=FindCmpCallbackAfterXP();
	//DbgPrint("CmCallbackListHead: %p\n",(PVOID)CmCallbackListHead);
	//enum callback address
	pNotify = CountCmpCallbackAfterXP((ULONG64*)CmCallbackListHead);
